...
//global Variables
extern uint32_t g_buttons_pressed;          // pressed button bit set to 1 in timer interrupt handler
extern uint8_t  g_auto_state;               // 6 control outputs that can be set either manually or automatically (0 -> manual, 1 -> auto)
extern uint32_t g_buttons_LEDs;             // pressed button bit set to 1 in timer interrupt handler
extern uint8_t  g_manual_button_state;
